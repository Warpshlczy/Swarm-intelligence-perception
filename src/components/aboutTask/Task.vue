  ...
  <div>
    <el-card class="taskBox" v-if="status === true">
      <div slot="header" class="header">
        ...
        <el-row>
          ...
          >
            <i
              :class="[
                { 'el-icon-star-on': collectStatus },
                { 'el-icon-star-off': !collectStatus },
              ]"
              style="color: #f95e5a"
              @click="Collection()"
            ></i>
      ...
      <div class="taskContainer">
        ...
        <el-row>
          <el-col :span="14" class="containerLeft">
            <div>
              <span>任务描述：</span>
              <div class="details">{{ taskData.details }}</div>
              <div>
                <el-image
                  v-for="item in taskData.imagesPath"
                  fit="cover"
                  :key="item"
                  class="imageClass"
                  :src="item"
            ...
            <div>
              <span>类型：</span> <span>{{ taskData.type }}</span>
            </div>
            <div>
              <span>发布时间：</span><span>{{ taskData.createTime }}</span>
            </div>
            <div>
              <span>发布者：</span><span>{{ taskData.publisherName }}</span>
            </div>
            <div>
              <span>开始提交时间：</span><span>{{ taskData.startTime }}</span>
            </div>
            <div>
              <span>结束提交时间：</span><span>{{ taskData.endTime }}</span>
            </div>
            <span>大任务进展：</span>
            <el-table
              :data="taskPercentage"
            ...
            >
              <el-table-column label="大任务" width="60">
                <template slot-scope="scope"
                  ><span v-if="scope.row.id === 1">总</span
                  ><span
                    v-else
                    style="
                      position: relative;
              ...
              <el-table-column label="大任务">
                <template
                  slot-scope="scope"
                  v-if="taskPercentage[0].children != []"
                >
                  <el-progress
                    :color="percentageSetColors"
                    v-if="scope.row.id === 1"
                    :percentage="(scope.row.passed / scope.row.maxPass) * 100"
                    :stroke-width="16"
                    :show-text="false"
                    class="progress"
                  ></el-progress>
                  <el-progress
                    ...
                    :stroke-width="16"
                    :show-text="false"
                    class="progress dividedProgress"
                  ></el-progress>
                  <span
                    v-if="scope.row.id === 1"
                    style="position: relative; top: -3px"
                    >{{ scope.row.passed }}/{{ scope.row.maxPass }}</span
                  >
                  <span v-else style="position: relative; top: -3px; left: 20px"
                    >{{ scope.row.passed }}/{{ scope.row.maxPass }}</span
                  >
                </template>
              </el-table-column>
          ...
          <el-col :span="10" class="containerRight">
            <div>
              <span>任务地点(请先点击选择一个任务地点网格再进行提交)：</span>
            </div>
            <iframe
              ref="iframe"
              name="selectAdress"
              src="static/任务地址.html"
              frameborder="0"
              scrolling="no"
              class="taskAdress"
            >
      ...
      <!-- <el-tooltip
        ...
        placement="top-end"> -->
      <!-- </el-tooltip> -->
      <TaskComment v-if="isOpenComment"></TaskComment>
    </el-card>
    <!-- 数据提交框 -->
    <el-dialog
      title="数据提交框"
      ...
      width="50%"
      @close="submitDialogClosed"
      top="5vh"
      v-if="status === true"
    >
      <!-- 内容主体区域 -->
      <el-form
        :model="dataSubmitForm"
        :rules="dataSubmitFormRules"
        ref="dataSubmitFormRef"
        label-width="80px"
        v-loading="isloading"
      >
        <el-form-item
          label="提交描述"
          prop="text"
          v-show="taskData.submitLimit.charAt(0) === '1'"
          :rules="[
            {
              required: taskData.submitLimit.charAt(0) === '1',
              message: '请输入数据的文字描述',
              trigger: 'blur',
            },
          ]"
        >
          <el-input
            type="textarea"
            v-model="dataSubmitForm.text"
            maxlength="200"
            show-word-limit
            rows="5"
            placeholder="请输入数据的文字描述"
            :disabled="flower"
          ></el-input>
        </el-form-item>
        <el-form-item
          label="数值"
          prop="number"
          v-show="taskData.submitLimit.charAt(1) === '1'"
          :rules="[
            {
              required: taskData.submitLimit.charAt(1) === '1',
              message: '请输入数值数据',
              trigger: 'blur',
            },
          ]"
        ...
        <el-form-item
          label="文件"
          prop="files"
          v-show="taskData.submitLimit.charAt(2) === '1'"
          :rules="[
            {
              required: taskData.submitLimit.charAt(2) === '1',
              message: '请提交文件数据',
              trigger: 'blur',
            },
          ]"
        ...
        <el-form-item
          label="地点"
          prop="place"
          v-show="taskData.submitLimit.charAt(3) === '1'"
        >
          <iframe
            name="selectAdress"
            ref="iframe1"
            src="static/选择地址.html"
            frameborder="0"
            scrolling="no"
            class="selectAdress"
          >
...
export default {
  ...
  data() {
    ...
    return {
      ...
      isOpenComment: false, // false：展示详情页，true：展示评论区
      submitLimit: [],
      queryInfo: {
        userId: this.userId,
  ...
  methods: {
    ...
    async getTask() {
      ...
      const name = await getPublisherName({
      ...
      if (res.data.data.imagesPath && res.data.data.imagesPath.length !== 0) {
        ...
        for (var i = 0; i < res.data.data.imagesPath.length; i++) {
          res.data.data.imagesPath[i] =
            "http://101.34.16.47/resource" + res.data.data.imagesPath[i];
        }
      }
      ...
      this.taskData.startTime = this.taskData.startTime
        .split(".")[0]
        .replace("T", " ");
      this.taskData.endTime = this.taskData.endTime
        .split(".")[0]
        .replace("T", " ");
      this.taskData.createTime = this.taskData.createTime
        .split(".")[0]
        .replace("T", " ");
      ...
      const res1 = await getSmallTask({
        bigTaskId: parseInt(this.$route.query.id),
      });
      ...
      for (let i = 0; i < this.coordinate.length; i++) {
        ...
        if (this.passedPercentage.length < this.coordinate.length) {
          if (data.currentPassed >= data.maxPassed)
            data.currentPassed = data.maxPassed;
          ...
          iframeWin.postMessage(
            {
              ...
              size: this.size,
              passedPercentage: this.passedPercentage,
            },
            "*"
          );
        });
