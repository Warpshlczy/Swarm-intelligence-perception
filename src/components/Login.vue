<template>
  <div class="login_container">
    <div class="login_box">
      ...
      <!-- 密码登录 -->
      <el-form v-if="!isMessage" ref="loginFormRef" :model="loginForm" :rules="loginFormRules" label-width="0px" class="login_form">
        <!-- 用户名 -->
        ...
        <el-form-item prop="password">
          <el-input v-model="loginForm.password" prefix-icon="el-icon-lock" placeholder="请输入密码" show-password @keydown.enter.native="passwordLogin"></el-input>
        </el-form-item>
        <el-form-item class="btns">
          <el-button type="primary" @click="passwordLogin" class="loginbutton">登录</el-button>
          <!-- <el-button type="info" @click="resetLoginForm">重置</el-button> -->
        </el-form-item>
        <el-form-item class="bottom">
          <el-switch v-model="loginForm.loginstate" active-text="记住我"></el-switch>
          <!-- <el-link type="primary" :underline="false" class="switch" @click="pushThemeMap">游客登录</el-link> -->
          <el-link type="primary" :underline="false" class="link" @click="pushMessageLogin">短信登录</el-link>
          <el-link type="primary" :underline="false" class="link" @click="pushResetPassword">忘记密码</el-link>
          <el-link @click="pushRegister" type="primary" :underline="false" class="link">注册新用户</el-link>
        </el-form-item>
      </el-form>
      <!-- 短信登录 -->
      <el-form v-if="isMessage" ref="loginFormRef" :model="loginForm" :rules="loginFormRules" label-width="0px" class="login_form">
        ...
        <el-form-item prop="verifyCode" class="verifyitem" >
          <el-input v-model="loginForm.verifyCode" prefix-icon="el-icon-message" placeholder="请输入验证码" class="verifyCode" @keydown.enter.native="messageLogin"></el-input>
        ...
        <el-form-item class="btns">
          <el-button type="primary" @click="messageLogin" class="loginbutton" >登录</el-button>
        </el-form-item>
        <el-form-item class="bottom">
          <el-switch v-model="loginForm.loginstate" active-text="记住我"></el-switch>
          <el-link type="primary" :underline="false" class="link" @click="pushPasswordLogin">密码登录</el-link>
          <el-link type="primary" :underline="false" class="link" @click="pushResetPassword">忘记密码</el-link>
          <el-link @click="pushRegister" type="primary" :underline="false" class="link">注册新用户</el-link>
        </el-form-item>
      </el-form>
...
<script>
import {userLoginByPassword, userLoginByCode, getRsa, getUserInfo, sendCode} from '@/api/user'
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.path !== '/' && from.path !== '/userInformation' && from.path !== '/resetpassword' && from.path !== '/resetphone') {
      next(vm => {
  ...
  methods: {
    ...
    window.document.cookie =
      'loginstate' + '=' + loginstate + ';path=/;expires=' + exdate.toGMTString()
    },
    pushMessageLogin() {
      this.isMessage = true
    },
    ...
    async passwordLogin() {
      ...
      this.$refs.loginFormRef.validate(async (valid) => {
        ...
        if (this.logindata.data.code === 200) {
          ...
          localStorage.setItem('Token', this.logindata.data.data)

          // 登录成功以后获取用户id并保存
          const user = await getUserInfo(this.logindata.data.data)
          localStorage.setItem('UserId', user.data.data.id)
          // 成功以后跳转到默认首页
          // console.log(user)
          if (user.data.data.home != null && user.data.data.home != 'null') {
      ...
      if (this.loginForm.loginstate === true) {
        this.setCookie(this.loginForm.phone, this.loginForm.password, this.loginForm.loginstate, 7)
      } else {
        this.setCookie('', '', '', 1)
      }
    ...
    async messageLogin() {
      ...
      this.$refs.loginFormRef.validate(async (valid) => {
        ...
        if (this.logindata.data.code === 200) {
          ...
          // 登录成功以后获取用户id并保存
          const userId = await getUserInfo(this.logindata.data.data)
    ...
    pushResetPassword() {
      if (this.loginForm.phone) this.$router.push({ path: '/resetpassword', query: {phone: this.loginForm.phone}})
